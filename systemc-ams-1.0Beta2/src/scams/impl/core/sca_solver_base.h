...
/*****************************************************************************
  ...
  Created on: 14.05.2009
   ...
   SVN Id            :  $Id: sca_solver_base.h 1187 2011-05-05 16:45:03Z karsten $
...
class sca_solver_base :  public sca_synchronization_obj_if, public ::sc_core::sc_object
{
  ...
  typedef  std::vector<sca_sync_port*>            sca_sync_portListT;
  typedef  std::vector<sca_sync_port*>::iterator  sca_sync_portListItT;
 ...
 public:
  ...
  void push_back_sync_port(
                            ...
                            sca_core::sca_implementation::sca_synchronization_obj_if::directionT      dir,
  ...
  const ::sc_core::sc_time& get_cluster_period() { return   cluster_period   ? *cluster_period   : not_valid_time;}
  const sc_dt::int64*& get_call_counter_ref() { return   call_counter;     }
  const bool*& get_allow_processing_access_flag_ref() { return   allow_processing_access_flag;     }
     ...
     * influence
     */
     ::sc_core::sc_time get_cluster_start_time();

    /** returns the current time when the object of the current cluster has
     *  been called
     */
     ::sc_core::sc_time get_current_time();
     ...
     * influence
     */
     ::sc_core::sc_time get_current_period();
     ...
     */
     long set_max_start_time(::sc_core::sc_time& ntime);
      ...
      * will be allways 0)
      */
      long set_max_period(::sc_core::sc_time& nperiod);
 ...
 private:
  ...
  //disabled
  sca_solver_base() : long_zero(0), not_valid_time(NOT_VALID_SCA_TIME()) {}

  //Synchronization layer interface for synchronization objects


  /** Implements @ref #class sca_synchronization_obj_if method */
  unsigned long get_number_of_sync_ports(directionT direction);

  /** Implements @ref #class sca_synchronization_obj_if method */
  sca_sync_port* get_sync_port(directionT direction, unsigned long number);
  ...
  void set_sync_data_references(cluster_sync_data& crefs,
...
private:
    ...
    //const values can be changed from sync-layer only
    const ::sc_core::sc_time*   cluster_period;
    const ::sc_core::sc_time*   cluster_start_time;
    ...
    class sca_sync_trace_handle
    {
     public:
        ...
        void resize(long n)
        {
          long old_size=time_points.size();
