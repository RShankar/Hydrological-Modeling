...
/*****************************************************************************
 ...
 SVN Version       :  $Revision: 1111 $
 SVN last checkin  :  $Date: 2011-01-20 21:40:26 +0100 (Thu, 20 Jan 2011) $
 SVN checkin by    :  $Author: karsten $
 SVN Id            :  $Id: sca_solver_base.cpp 1111 2011-01-20 20:40:26Z karsten $
...
sca_solver_base::sca_solver_base(const char* solver_name,
	...
	::sc_core::sc_object(solver_name),
	         associated_module_list(associated_modules),associated_channel_list(associated_channels),
	        long_zero(0),
	        not_valid_time(NOT_VALID_SCA_TIME()),
	        cluster_period(NULL), cluster_start_time(NULL),
			call_counter(NULL), allow_processing_access_flag(NULL),
			calls_per_period(NULL)
{
	if (solver_name == NULL)
		sync_obj_name = "unnamed_solver";
	else
		sync_obj_name = solver_name;
	...
	timestep_calculated=sc_core::SC_ZERO_TIME;
...
void sca_solver_base::push_back_sync_port(sca_port_base* port, directionT dir,
		sca_interface* ch, unsigned long& rate, unsigned long& delay,
		::sc_core::sc_time& T, ::sc_core::sc_time& t0)
{
	sca_core::sca_implementation::sca_sync_port* con =
			new sca_core::sca_implementation::sca_sync_port;

	con->parent_obj = this;
	con->port = port;

	if ((ch == NULL)&& (dir==TO_ANALOG) | (dir==FROM_ANALOG))
	{
	...
	if( (con->channel == NULL) && (dir==TO_ANALOG) | (dir==FROM_ANALOG))
	{
		std::ostringstream str;
		//error should not be possible, due it has to be checked earlier ???
		str
	...
	{
	case TO_ANALOG:
		con->sync_port_number = to_analog.size();
		to_analog.push_back(con);
		break;
	case FROM_ANALOG:
		con->sync_port_number = from_analog.size();
		from_analog.push_back(con);
		break;
	case TO_SYSTEMC:
		con->sync_port_number = to_systemc.size();
		to_systemc.push_back(con);
		break;
	case FROM_SYSTEMC:
		con->sync_port_number = from_systemc.size();
...
unsigned long sca_solver_base::get_number_of_sync_ports(directionT direction)
{
	switch (direction)
	{
	case TO_ANALOG:
		return to_analog.size();
	case FROM_ANALOG:
		return from_analog.size();
	case TO_SYSTEMC:
		return to_systemc.size();
	case FROM_SYSTEMC:
		return from_systemc.size();
	default:
		std::ostringstream str;
		str << "Internal error undefined channel direction: " << __FILE__
				<< "  line: " << __LINE__ << std::endl;
...
sca_solver_base::sca_sync_port*
sca_solver_base::get_sync_port(directionT direction, unsigned long number)
{
...
void sca_solver_base::push_back_to_analog(sca_port_base* port,
		...
		::sc_core::sc_time& T, ::sc_core::sc_time& t0)
{
	push_back_sync_port(port, TO_ANALOG, sig, rate, delay, T, t0);
}
...
void sca_solver_base::push_back_from_analog(sca_port_base* port,
		...
		::sc_core::sc_time& T, ::sc_core::sc_time& t0)
{
	push_back_sync_port(port, FROM_ANALOG, sig, rate, delay, T, t0);
}
...
void sca_solver_base::push_back_to_systemc(sca_port_base* port,
		...
		::sc_core::sc_time& T, ::sc_core::sc_time& t0)
{
	push_back_sync_port(port, TO_SYSTEMC, sig, rate, delay, T, t0);
}
...
void sca_solver_base::push_back_from_systemc(sca_port_base* port,
		...
		::sc_core::sc_time& T, ::sc_core::sc_time& t0)
{
	push_back_sync_port(port, FROM_SYSTEMC, sig, rate, delay, T, t0);
}
...
{
	long idl = sync_trace_handles.size();
		...
		::sc_core::sc_time next_time, sca_sync_value_handle_base& val_handle)
{
	if (ctime < (::sc_core::sc_time_stamp()))
	{
		...
		str << "sca-de synchronization failed in: "
				<< cluster_id << " " << __FILE__ << " line: " << __LINE__
	...
	if (((*cluster_resume_event_time) > next_time)
			|| ((*cluster_resume_event_time) <= ::sc_core::sc_time_stamp()))
	...
	{
		cluster_resume_event->cancel();//remove due earliest notification wins (LRM)
		cluster_resume_event->notify(next_time - ::sc_core::sc_time_stamp());
		(*cluster_resume_event_time) = next_time;
	}
...
void sca_solver_base::set_sync_data_references(cluster_sync_data& crefs,
		object_sync_data& orefs)
{
	cluster_period = &(crefs.cluster_period);
	cluster_start_time = &(crefs.cluster_start_time);
	next_cluster_period = &(crefs.next_cluster_period);
	next_cluster_start_time = &(crefs.next_cluster_start_time);
	cluster_traces = &(crefs.traces);
	request_new_start_time = &(crefs.request_new_start_time);
	request_new_period = &(crefs.request_new_period);

	cluster_resume_event = &(crefs.cluster_resume_event);
	cluster_resume_event_time = &(crefs.cluster_resume_event_time);

	reactivity_events = &(crefs.reactivity_events);

	call_counter = &(orefs.call_counter);
	allow_processing_access_flag=&(orefs.allow_processing_access_flag);
	calls_per_period = &(orefs.calls_per_period);
		...
		sca_sync_value_handle_base& handle)
{
	...
	if ((next_time != NOT_VALID_SCA_TIME()) && (next_time
			< (*cluster_resume_event_time)))
	{
		(*cluster_resume_event_time) = next_time;
		(*cluster_resume_event).notify(next_time - ::sc_core::sc_time_stamp());
	}

	handle.set_id(sca_sync_write_handles.size());
...
sc_core::sc_time sca_solver_base::get_cluster_start_time()
{
	return *cluster_start_time;
}

//////////////////////////////////////

sc_core::sc_time sca_solver_base::get_current_time()
{
	if (call_counter == NULL)
		...
		return (*cluster_start_time) + (((*call_counter) % (*calls_per_period))
				* ((*cluster_period)) / (*calls_per_period));
}

//////////////////////////////////////

sc_core::sc_time sca_solver_base::get_current_period()
{
	if ((cluster_period == NULL) || (calls_per_period == NULL))
		return NOT_VALID_SCA_TIME();
	return (*cluster_period) / (*calls_per_period);
}

//////////////////////////////////////

long sca_solver_base::set_max_start_time(::sc_core::sc_time& ntime)
{
	...
	long shift = ((*call_counter) + (*calls_per_period) - 1)
			% (*calls_per_period);
	if (shift)
		ntime = ::sc_core::sc_time_stamp() + (shift * (*cluster_period))
				/ (*calls_per_period);

	if (!(*request_new_start_time) || ((*next_cluster_start_time) > ntime))
	{
		(*request_new_start_time) = true;
		(*next_cluster_start_time) = ntime;
	}
...
long sca_solver_base::set_max_period(::sc_core::sc_time& nperiod)
{
	long shift = (*call_counter + (*calls_per_period) - 1)
			% (*calls_per_period);

	if (!(*request_new_period) || ((*next_cluster_period) > nperiod))
	{
		(*request_new_period) = true;
		(*next_cluster_period) = nperiod;
	}
		...
		const ::sc_core::sc_event& ev)
{
	...
	if (shift)
		earliest_time = ::sc_core::sc_time_stamp()
				+ (shift * (*cluster_period)) / (*calls_per_period);

	reactivity_events->push_back(&ev);
...
{
	cluster_traces->push_back(&tr_obj);
}
