...
sca_transmission_line::sca_transmission_line(sc_core::sc_module_name,
		double z0_, sca_core::sca_time delay_, double delta0_) :
...
void sca_transmission_line::init_delays()
{
	...
	if (nsample == 0)
	{
		...
		B(nadd1, a2) =   k_diss * (dt_sec - delay_sec) / dt_sec; //va1b1= k*va2b2(..)
		B(nadd1, b2) =  -k_diss * (dt_sec - delay_sec) / dt_sec;

		B(nadd1, nadd2) = k_diss * z0_val * (dt_sec - delay_sec) / dt_sec; // + z0ia2b2


		B(nadd2, a1) =  k_diss * (dt_sec - delay_sec) / dt_sec; //va2b2= k*va1b1(..)
		B(nadd2, b1) = -k_diss * (dt_sec - delay_sec) / dt_sec;

		B(nadd2, nadd1) = k_diss * z0_val * (dt_sec - delay_sec) / dt_sec; // + z0ia1b1
...
void sca_transmission_line::matrix_stamps()
{
	...
	B(nadd1,nadd1) = z0_val;  // z0*ia1 = ...
	B(nadd1, a1)   = -1.0;    // v(a1,b1) ...
	B(nadd1, b1)   =  1.0;
	B(a1, nadd1)   =  1.0;    //i(a1)
	B(b1, nadd1)   = -1.0;    //i(b1)
	...
	B(nadd2,nadd2) = z0_val; // z0*ia2 = ...
	B(nadd2, a2)   = -1.0;   // v(a2,b2) = ...
	B(nadd2, b2)   =  1.0;
	B(a2, nadd2)   =  1.0;   // i(a2)
	B(b2, nadd2)   = -1.0;   // i(b2)
	...
	if(!sca_ac_analysis::sca_ac_is_running())
	{
		...
		add_post_solve_method(SCA_VMPTR(sca_transmission_line::post_solve));

		init_delays();

		if(!initialized)
		{
			add_pre_solve_method(SCA_VMPTR(sca_transmission_line::pre_solve));
		}
	}
	else
	{
		...
		B(nadd1, a2) =   k_diss   * cos(-w*delay_sec); //va1b1= k*va2b2(..)
		A(nadd1, a2) =   k_diss/w * sin(-w*delay_sec);

		B(nadd1, b2) =  -k_diss *   cos(-w*delay_sec);
		A(nadd1, b2) =  -k_diss/w * sin(-w*delay_sec);

		B(nadd1, nadd2) = k_diss   * z0_val * cos(-w*delay_sec); // + z0ia2b2
		A(nadd1, nadd2) = k_diss/w * z0_val * sin(-w*delay_sec);

		B(nadd2, a1) =  k_diss   * cos(-w*delay_sec); //va2b2= k*va1b1(..)
		A(nadd2, a1) =  k_diss/w * sin(-w*delay_sec);

		B(nadd2, b1) = -k_diss   * cos(-w*delay_sec);
		A(nadd2, b1) = -k_diss/w * sin(-w*delay_sec);

		B(nadd2, nadd1) = k_diss   * z0_val * cos(-w*delay_sec); // + z0ia1b1
