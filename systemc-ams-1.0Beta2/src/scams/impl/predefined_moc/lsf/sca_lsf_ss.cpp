...
/*****************************************************************************
  ...
  Created on: 08.01.2010
   ...
   SVN Id            :  $Id: sca_lsf_ss.cpp 1102 2011-01-02 22:18:22Z karsten $
...
void sca_ss::matrix_stamps()
{
	...
	for(unsigned long i=0;i<add_eq.size();i++)
	{
		...
		else
		{
			...
			add_post_solve_method(SCA_VMPTR(sca_ss::dc_step_finish));
		}
	}
	...
	if (delay.get() != sc_core::SC_ZERO_TIME)
	{
		...
		if (!sca_ac_analysis::sca_ac_is_running())
		{
			...
			if (!initialized)
			{
				add_pre_solve_method(SCA_VMPTR(sca_ss::pre_solve));
			}

			init_delay();
			q(y).add_element(SCA_MPTR(sca_ss::q_t), this);
			add_post_solve_method(SCA_VMPTR(sca_ss::post_solve));
...
void sca_ss::dc_step_finish()
{
	...
	remove_post_solve_method(SCA_VMPTR(sca_ss::dc_step_finish));
