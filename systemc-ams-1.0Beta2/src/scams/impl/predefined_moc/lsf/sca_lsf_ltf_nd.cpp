...
/*****************************************************************************
 ...
 SVN Id            :  $Id: sca_lsf_ltf_nd.cpp 1102 2011-01-02 22:18:22Z karsten $
...
sca_ltf_nd::sca_ltf_nd(sc_core::sc_module_name, const sca_util::sca_vector<
	...
	x("x"), y("y"), num("num", num_), den("den", den_), delay("delay",
			sc_core::SC_ZERO_TIME), k("k", k_)
{
	dc_init=true;
}
...
sca_ltf_nd::sca_ltf_nd(sc_core::sc_module_name, const sca_util::sca_vector<
		...
		sca_core::sca_time delay_, double k_) :
	x("x"), y("y"), num("num", num_), den("den", den_), delay("delay", delay_),
			k("k", k_)
{
	dc_init=true;
}
...
void sca_ltf_nd::matrix_stamps()
{
	...
	if (delay.get() != sc_core::SC_ZERO_TIME)
	{
		...
		if (!sca_ac_analysis::sca_ac_is_running())
		{
			...
			if (!initialized)
			{
				add_pre_solve_method(SCA_VMPTR(sca_ltf_nd::pre_solve));
			}

			init_delay();
			q(y).add_element(SCA_MPTR(sca_ltf_nd::q_t), this);
			add_post_solve_method(SCA_VMPTR(sca_ltf_nd::post_solve));
	...
	if(dc_init && !sca_ac_analysis::sca_ac_is_running())
	{
		add_post_solve_method(SCA_VMPTR(sca_ltf_nd::dc_step_finish));
	}
...
void sca_ltf_nd::dc_step_finish()
{
	...
	remove_post_solve_method(SCA_VMPTR(sca_ltf_nd::dc_step_finish));
