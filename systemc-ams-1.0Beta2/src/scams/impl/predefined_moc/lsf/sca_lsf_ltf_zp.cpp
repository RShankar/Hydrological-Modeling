...
/*****************************************************************************
 ...
 SVN Id            :  $Id: sca_lsf_ltf_zp.cpp 1102 2011-01-02 22:18:22Z karsten $
...
sca_ltf_zp::sca_ltf_zp(sc_core::sc_module_name, const sca_util::sca_vector<
		sca_util::sca_complex>& zeros_, const sca_util::sca_vector<
		sca_util::sca_complex>& poles_, double k_) :
	x("x"), y("y"), zeros("zeros", zeros_), poles("poles", poles_), delay(
			"delay", sc_core::SC_ZERO_TIME), k("k", k_)
{
	dc_init=true;
}

sca_ltf_zp::sca_ltf_zp(sc_core::sc_module_name, const sca_util::sca_vector<
		sca_util::sca_complex>& zeros_,
...
//			  << den_ltf << std::endl << "k: " << k.get() << std::endl;
	...
	if (delay.get() != sc_core::SC_ZERO_TIME)
	{
		...
		if (!sca_ac_analysis::sca_ac_is_running())
		{
			...
			if (!initialized)
			{
				add_pre_solve_method(SCA_VMPTR(sca_ltf_zp::pre_solve));
			}

			init_delay();
			q(y).add_element(SCA_MPTR(sca_ltf_zp::q_t), this);
			add_post_solve_method(SCA_VMPTR(sca_ltf_zp::post_solve));
	...
	if(dc_init && !sca_ac_analysis::sca_ac_is_running())
	{
		add_post_solve_method(SCA_VMPTR(sca_ltf_zp::dc_step_finish));
	}
}
...
void sca_ltf_zp::dc_step_finish()
{
	...
	remove_post_solve_method(SCA_VMPTR(sca_ltf_zp::dc_step_finish));
