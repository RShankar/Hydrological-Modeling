...
/*****************************************************************************
  ...
  Created on: 15.12.2009
   ...
   SVN Version       :  $Revision: 1038 $
   SVN last checkin  :  $Date: 2010-08-15 21:59:10 +0200 (Sun, 15 Aug 2010) $
   SVN checkin by    :  $Author: karsten $
   SVN Id            :  $Id: sca_tdf_ct_ltf_zp_proxy.cpp 1038 2010-08-15 19:59:10Z karsten $
		...
		const sca_util::sca_vector<sca_util::sca_complex>& poles)
{
	...
	if (zeros.get_access_flag() || &zeros != zeros_old_ref)
	{
		...
		if (zeros.length() != zeros_old.length())
		{
			zeros_old = zeros;
			poles_old = poles;
			poles_old_ref = &poles;
	...
	if (poles.get_access_flag() || &poles != poles_old_ref)
	{
		...
		if (poles.length() != poles_old.length())
		{
			zeros_old = zeros;
			poles_old = poles;
			zeros_old_ref = &zeros;
