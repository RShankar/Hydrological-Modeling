...
/*****************************************************************************
 ...
 SVN Version       :  $Revision: 1129 $
 SVN last checkin  :  $Date: 2011-01-29 17:50:39 +0100 (Sat, 29 Jan 2011) $
 SVN checkin by    :  $Author: karsten $
 SVN Id            :  $Id: sca_trace_buffer.cpp 1129 2011-01-29 16:50:39Z karsten $
		...
		value_buffer*& values)
{
	...
	for (long i = 0; i < number_of_traces; ++i)
	{
		...
		if ((buf_it->second)[i] == NULL)
		{
			...
			else //no last_value available -> we propagate the first value to time zero
			{
				if (init_values[i] == NULL)
				{
					std::ostringstream str;
					str << "Internal error - every trace must write at least "
							<< "one value before writing to file " << i
							<< std::endl;
					SC_REPORT_ERROR("SystemC-AMS",str.str().c_str());
				}
				//first value is propagated backward
