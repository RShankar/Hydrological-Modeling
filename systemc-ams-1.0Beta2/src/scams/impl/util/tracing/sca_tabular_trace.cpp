...
/*****************************************************************************
 ...
 SVN Id            :  $Id: sca_tabular_trace.cpp 1031 2010-06-04 16:26:43Z karsten $
...
void sca_tabular_trace::set_mode_impl(const sca_util::sca_trace_mode_base& mode)
{
	...
	{
		std::cout << " no interpol mode" << std::endl;
    ...
    sca_util::sca_matrix<sca_util::sca_complex >& tr_matrix)
{
    ...
    long n_src;
    if(noise_all_flag)
        n_src=tr_matrix.n_rows();
    else
        n_src=1; //only overall noise

    for(unsigned long point=0;point<tr_matrix.n_cols();point++)
    {
        for(long nsrc=0;nsrc<n_src;nsrc++)
        {
        	double val1, val2;
        	sca_util::sca_complex res=tr_matrix(nsrc,point);
