...
/*****************************************************************************
  ...
  Created on: 03.01.2010
   ...
   SVN Id            :  $Id: sca_ac_functions.cpp 1186 2011-05-05 15:37:00Z karsten $
...
{
	return pow( sca_util::SCA_COMPLEX_J * sca_ac_w(),n);
}

//LRM clause 5.2.2.8.
sca_util::sca_complex sca_ac_z(long n, const sca_core::sca_time& tstep)
{
	return exp( sca_util::SCA_COMPLEX_J * (sca_ac_w()*(double(n)*tstep.to_seconds())) );
}
...
sca_util::sca_complex sca_ac_z(long n)
{
	...
	double tstep=tdf_obj->get_timestep().to_seconds();
...
sca_util::sca_complex sca_ac_ltf_zp(const sca_util::sca_vector<
		sca_util::sca_complex>& zeros, const sca_util::sca_vector<
		sca_util::sca_complex>& poles, sca_core::sca_time ct_delay,
		const sca_util::sca_complex& input, double k)
{
				...
				sca_util::sca_complex>& input)
{
    ...
    sca_util::sca_matrix<sca_util::sca_complex> ac(a.n_rows(),a.n_cols());
