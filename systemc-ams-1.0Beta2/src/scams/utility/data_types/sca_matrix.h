...
/*****************************************************************************
 ...
 SVN Version       :  $Revision: 1112 $
 SVN last checkin  :  $Date: 2011-01-20 21:55:28 +0100 (Thu, 20 Jan 2011) $
 SVN checkin by    :  $Author: karsten $
 SVN Id            :  $Id: sca_matrix.h 1112 2011-01-20 20:55:28Z karsten $
...
public:
	...
	const sca_util::sca_matrix<T>& operator=(sca_core::sca_assign_from_proxy<
			sca_util::sca_matrix<T> >&);
...
template<class T>
unsigned long sca_matrix<T>::n_rows() const
{
	return base_type::dimy();
}

template<class T>
unsigned long sca_matrix<T>::n_cols() const
{
	return base_type::dimx();
}

template<class T>
T& sca_matrix<T>::operator()(unsigned long row, unsigned long col)
{
	return base_type::operator()(row,col);
}

template<class T>
const T& sca_matrix<T>::operator()(unsigned long row, unsigned long col) const
{
	return base_type::operator()(row,col);
}

template<class T>
const sca_util::sca_matrix<T>& sca_matrix<T>::operator=(const sca_util::sca_matrix<T>& matrix_)
{
...
template<class T>
const sca_util::sca_matrix<T>& sca_matrix<T>::operator=(sca_core::sca_assign_from_proxy<
...
template<class T>
const std::string sca_matrix<T>::to_string() const
{
...
template<class T>
void sca_matrix<T>::print(std::ostream& str) const
{
	str << *this;
}
